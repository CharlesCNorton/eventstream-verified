...
| h :: t0 ->
  if (=) h.ev_id e.ev_id
  then if (<=) h.ev_seq e.ev_seq then e :: t0 else h :: t0
  else h :: (replace_or_add e t0)
...
(** val replace_or_add_map : event -> event IdMap.t -> event IdMap.t **)

let replace_or_add_map e m =
  match IdMap.find e.ev_id m with
  | Some old -> if (<=) old.ev_seq e.ev_seq then IdMap.add e.ev_id e m else m
  | None -> IdMap.add e.ev_id e m

(** val apply_events_map : event list -> event IdMap.t -> event IdMap.t **)

let rec apply_events_map sorted m =
  match sorted with
  | [] -> m
  | e :: rest ->
    (match e.ev_kind with
     | Cancel -> apply_events_map rest (IdMap.remove e.ev_id m)
     | _ -> apply_events_map rest (replace_or_add_map e m))

(** val map_values : event IdMap.t -> event list **)
...
| [] -> false
| h :: t0 -> (||) ((=) n h) (mem_nat n t0)

(** val dedup_nat : int list -> int list **)

let rec dedup_nat = function
| [] -> []
| h :: t0 -> if mem_nat h t0 then dedup_nat t0 else h :: (dedup_nat t0)
