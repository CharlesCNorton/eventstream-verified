...
(*     raw event stream reduced to a canonical truth stream.  Canonicalize    *)
(*     extracts pure and idempotent; fold_stream matches batch by             *)
(*     construction.  Serves as the replay-safe ingestion layer for any       *)
(*     data product or analytics pipeline that must be bit-for-bit            *)
(*     reproducible.                                                          *)
(*                                                                            *)
(*     Proven: determinism, idempotence, sort correctness, no cancel          *)
(*     leakage, unique ids, semantic spec, output-subset, online-batch        *)
(*     equivalence, chunked processing.                                       *)
(*                                                                            *)
